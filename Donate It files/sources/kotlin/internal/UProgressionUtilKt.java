...
public final class UProgressionUtilKt {
    ...
    private static final int m894differenceModuloWZ9TVnA(int a, int b, int c) {
    ...
    private static final long m895differenceModulosambcqE(long a, long b, long c) {
    ...
    public static final int m897getProgressionLastElementNkh28Cs(int start, int end, int step) {
        if (step > 0) {
            if (UnsignedKt.uintCompare(start, end) < 0) {
                return UInt.m88constructorimpl(end - m894differenceModuloWZ9TVnA(end, start, UInt.m88constructorimpl(step)));
            }
        } else if (step >= 0) {
            throw new IllegalArgumentException("Step is zero.");
        } else if (UnsignedKt.uintCompare(start, end) > 0) {
            return UInt.m88constructorimpl(m894differenceModuloWZ9TVnA(start, end, UInt.m88constructorimpl(-step)) + end);
        }
        return end;
    }

    /* renamed from: getProgressionLastElement-7ftBX0g  reason: not valid java name */
    public static final long m896getProgressionLastElement7ftBX0g(long start, long end, long step) {
        if (step > 0) {
            if (UnsignedKt.ulongCompare(start, end) < 0) {
                return ULong.m157constructorimpl(end - m895differenceModulosambcqE(end, start, ULong.m157constructorimpl(step)));
            }
        } else if (step >= 0) {
            throw new IllegalArgumentException("Step is zero.");
        } else if (UnsignedKt.ulongCompare(start, end) > 0) {
            return ULong.m157constructorimpl(m895differenceModulosambcqE(start, end, ULong.m157constructorimpl(-step)) + end);
        }
        return end;
    }
}
