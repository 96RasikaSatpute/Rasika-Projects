...
public class MessagingAnalytics {
    ...
    static int getPriority(Intent intent) {
        ...
        if (stringExtra == null) {
            if (DiskLruCache.VERSION_1.equals(intent.getStringExtra(Constants.MessagePayloadKeys.PRIORITY_REDUCED_V19))) {
                return 2;
            }
    ...
    private static void setUserPropertyIfRequired(Intent intent) {
        if (intent != null) {
            if (DiskLruCache.VERSION_1.equals(intent.getStringExtra(Constants.AnalyticsKeys.TRACK_CONVERSIONS))) {
                AnalyticsConnector analyticsConnector = (AnalyticsConnector) FirebaseApp.getInstance().get(AnalyticsConnector.class);
                if (Log.isLoggable(Constants.TAG, 3)) {
    ...
    public static boolean shouldUploadScionMetrics(Intent intent) {
        ...
        return DiskLruCache.VERSION_1.equals(intent.getStringExtra(Constants.AnalyticsKeys.ENABLED));
    }
}
