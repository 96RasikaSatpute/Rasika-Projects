...
public class AdaptiveStreamBuffer {
    ...
    public AdaptiveStreamBuffer(InputStream source2, int initialBufferSize) {
        this.source = source2;
    ...
    public int advance(int bytes) throws IOException {
        ...
        while (bytesAdvanced < bytes) {
            int currentSkip = (int) this.source.skip((long) (bytes - bytesAdvanced));
            if (currentSkip > 0) {
    ...
    private int resize(int targetSize) {
        ...
        long availableMemory = runtime2.maxMemory() - (runtime2.totalMemory() - runtime2.freeMemory());
