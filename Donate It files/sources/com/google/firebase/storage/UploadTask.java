...
public class UploadTask extends StorageTask<TaskSnapshot> {
    ...
    UploadTask(StorageReference targetRef, StorageMetadata metadata, byte[] bytes) {
        ...
        this.mTotalByteCount = (long) bytes.length;
    ...
    UploadTask(StorageReference targetRef, StorageMetadata metadata, Uri file, Uri existingUploadUri) {
        ...
        this.mSender = new ExponentialBackoffSender(targetRef.getApp().getApplicationContext(), authProvider, storage.getMaxUploadRetryTimeMillis());
        long size = -1;
        try {
            ContentResolver resolver = targetRef.getStorage().getApp().getApplicationContext().getContentResolver();
            try {
                ParcelFileDescriptor fd = resolver.openFileDescriptor(file, "r");
                if (fd != null) {
                    size = fd.getStatSize();
                    fd.close();
                }
    ...
    UploadTask(StorageReference targetRef, StorageMetadata metadata, InputStream stream) {
        ...
        this.mUri = null;
        this.mSender = new ExponentialBackoffSender(targetRef.getApp().getApplicationContext(), authProvider, targetRef.getStorage().getMaxUploadRetryTimeMillis());
    }
    ...
    private void beginResumableUpload() {
        ...
        if (this.mUri != null && TextUtils.isEmpty(mimeType)) {
            mimeType = this.mStorageRef.getStorage().getApp().getApplicationContext().getContentResolver().getType(this.mUri);
        }
    ...
    private void uploadChunk() {
        try {
            ...
            this.mBytesUploaded.getAndAdd((long) bytesToUpload);
