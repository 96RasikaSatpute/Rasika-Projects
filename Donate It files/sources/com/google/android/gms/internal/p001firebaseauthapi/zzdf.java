package com.google.android.gms.internal.p001firebaseauthapi;

import java.security.GeneralSecurityException;

/* renamed from: com.google.android.gms.internal.firebase-auth-api.zzdf  reason: invalid package */
/* compiled from: com.google.firebase:firebase-auth@@20.0.4 */
final class zzdf extends zzas<zzam, zzhe> {
    zzdf(Class cls) {
        super(cls);
    }

    public final /* bridge */ /* synthetic */ Object zzb(Object obj) throws GeneralSecurityException {
        zzhe zzhe = (zzhe) obj;
        zzgy zzb = zzhe.zzb();
        zzhh zza = zzb.zza();
        return new zzki(zzkl.zzf(zzdn.zzc(zza.zza()), zzhe.zzc().zzp(), zzhe.zzd().zzp()), zza.zzc().zzp(), zzdn.zzb(zza.zzb()), zzdn.zzd(zzb.zzc()), new zzdo(zzb.zzb().zza()));
    }
}
