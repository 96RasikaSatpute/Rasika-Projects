...
class zzabz<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    private final int zza;
    /* access modifiers changed from: private */
    public List<zzabw> zzb = Collections.emptyList();
    /* access modifiers changed from: private */
    public Map<K, V> zzc = Collections.emptyMap();
    private boolean zzd;
    private volatile zzaby zze;
    private Map<K, V> zzf = Collections.emptyMap();

    /* synthetic */ zzabz(int i, zzabs zzabs) {
        this.zza = i;
    }
    ...
    public final V zzk(int i) {
        ...
        if (!this.zzc.isEmpty()) {
            ...
            List<zzabw> list = this.zzb;
            Map.Entry entry = (Map.Entry) it.next();
            list.add(new zzabw(this, (Comparable) entry.getKey(), entry.getValue()));
            it.remove();
        }
        return value;
    }
    ...
    public final Set<Map.Entry<K, V>> entrySet() {
        if (this.zze == null) {
            this.zze = new zzaby(this, (zzabs) null);
        }
        return this.zze;
    }
    ...
    public final boolean equals(Object obj) {
        ...
        int zzc2 = zzc();
        if (zzc2 != zzabz.zzc()) {
            return entrySet().equals(zzabz.entrySet());
        }
        for (int i = 0; i < zzc2; i++) {
            if (!zzd(i).equals(zzabz.zzd(i))) {
                return false;
            }
        }
        if (zzc2 != size) {
            return this.zzc.equals(zzabz.zzc);
        }
        return true;
    }
    ...
    public final V put(K k, V v) {
        ...
        int i2 = this.zza;
        if (size == i2) {
            zzabw remove = this.zzb.remove(i2 - 1);
            zzn().put(remove.zza(), remove.getValue());
        }
        this.zzb.add(i, new zzabw(this, k, v));
        return null;
    }
}
